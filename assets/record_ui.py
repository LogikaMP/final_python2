...
class RecordsFrame(CTkFrame):
    def __init__(self, app):
        ...
        CTkButton(self,width=60,height=40,bg_color="#3c80bc",color_color="#061076",text="⬅️",
        ...
        CTkScrollableFrame(fon="transparent").pack(fill="both", expand=True)
    ...
    def add_records(self):
        ...
        for ap in file:
            ...
            L=CTkLabel(self.scroll,text=ap,color="#b89bc6",font=("Arial",36))

            #НОВЕ: кріпимо як сітку (grid) ряд = і, колонка 0, відступи 50, 10
            L.grid(row=i,colmn= 0,padx= 50,pady = 10)
            ...
            CTkButton(text="🔊",w=30,h=30,color="#3c80bc",command=lambda file=file,
             i = i:self.play(file,i)).grid( row=i,colmn=2,w=20,h=10)

            #НОВЕ: додати кнопку грати до цього запису на скролінг 
            # текст ="🗑️", розмір 30 на 30, колір "#3c80bc" 
            # команада lambda file=file : self.del_sound(file)
            # кріпимо - ряд = і, колонка 2 вітсупи 20 10
            CTkButton(text="🗑️",w=30,h=30,color="#3c80bc",command=lambda file=file,
             i = i:self.play(file,i)).grid( row=i,colmn=2,w=20,h=10)
