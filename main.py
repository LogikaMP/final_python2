...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.level = 0
        self.mode = settings_data["modes"][0]
    ...
    def rebuild_main_frame(self):
        # Очистити вміст фрейму і побудувати головний UI заново з поточною темою
        ...
        ctk.CTkButton(self.frame, text="Старт", text_color=self.temu["text"],
                      ...
                      border_width=5, command=self.start).pack(pady=20)

        ctk.CTkButton(self.frame, text="Налаштування", text_color=self.temu["text"],
                      fg_color=self.temu["button"], corner_radius=25,
                      font=("Arial", 16), border_color=self.temu["button_hover"],
                      border_width=5, command=self.setting).pack(pady=20)
